...
class WCFM_Video_Controller {
	...
	public function processing() {
		...
		if ( isset( $_POST['video_status'] ) && ! empty( $_POST['video_status'] ) ) {
			$args['post_status'] = $_POST['video_status'];
		}
		...
		if ( ! empty( $wcfm_video_array ) ) {
			...
			foreach ( $wcfm_video_array as $wcfm_video_single ) {
				...
				if ( get_the_post_thumbnail_url( $wcfm_video_single->ID ) ) {
					$f_image = '<img width="40" height="40" class="attachment-thumbnail size-thumbnail wp-post-image" src="' . get_the_post_thumbnail_url( $wcfm_video_single->ID ) . '" />';
				} else {
					$f_image = '';
				}
				...
				if ( apply_filters( 'wcfm_is_allow_edit_video', true ) ) {
					$wcfm_video_json_arr[ $index ][] = '<a href="' . get_wcfm_cpt_manage_url( 'video', $wcfm_video_single->ID ) . '">' . $f_image . '</a>';
				} else {
					$wcfm_video_json_arr[ $index ][] = $f_image;
				}
				...
				} elseif ( $wcfm_video_single->post_status == 'publish' ) {
