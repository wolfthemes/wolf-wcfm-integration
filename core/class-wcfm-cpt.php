...
if(!function_exists('get_wcfm_video_url')) {
	function get_wcfm_video_url( $video_status = '' ) {
		...
		$get_wcfm_video_url = wcfm_get_endpoint_url( WCFM_VIDEO_CPT_SLUG, '', $wcfm_page );
		if($video_status) $get_wcfm_video_url = add_query_arg( 'video_status', $video_status, $get_wcfm_video_url );
		return apply_filters( 'wcfm_video_url', $get_wcfm_video_url );
	}
}

if(!function_exists('get_wcfm_video_manage_url')) {
	function get_wcfm_video_manage_url( $video_id = '' ) {
		global $WCFM;
		$wcfm_page = get_wcfm_page();
		$get_wcfm_video_manage_url = wcfm_get_endpoint_url( WCFM_VIDEO_CPT_SLUG . '-manage', $video_id, $wcfm_page );
		return apply_filters( 'wcfm_video_manage_url', $get_wcfm_video_manage_url );
	}
}
...
if(!function_exists('get_wcfm_cpt1_url')) {
	function get_wcfm_cpt1_url( $cpt1_status = '' ) {
		global $WCFM;
		$wcfm_page = get_wcfm_page();
		$get_wcfm_cpt1_url = wcfm_get_endpoint_url( WCFM_VIDEO_CPT_SLUG, '', $wcfm_page );
...
if(!function_exists('get_wcfm_cpt1_manage_url')) {
	function get_wcfm_cpt1_manage_url( $cpt1_id = '' ) {
		global $WCFM;
		$wcfm_page = get_wcfm_page();
		$get_wcfm_cpt1_manage_url = wcfm_get_endpoint_url( WCFM_VIDEO_CPT_SLUG . '-manage', $cpt1_id, $wcfm_page );
