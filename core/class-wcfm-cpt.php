...
class Wolf_WCFM_CPT_Module {
	...
	public function __construct( array $args ) {
		...
		add_filter( 'wcfm_query_vars', array( $this, 'add_query_vars' ), 50 );
		add_filter( 'wcfm_endpoint_title', array( $this, 'endpoint_title' ), 20, 2 );

		add_action( 'init', array( $this, 'init' ), 20 );

		add_filter( 'wcfm_endpoints_slug', array( $this, 'add_endpoints_slug' ) );
		...
		add_filter( 'wcfm_menus', array( $this, 'add_menu' ) );
	...
	public function load_styles( $end_point ) {
		...
		switch ( $end_point ) {
			case 'wcfm-' . $this->slug:
				wp_enqueue_style( 'wcfm_' . $this->slug . '_css', $this->plugin_url . 'css/' . $this->slug . '/wcfm-style-' . $this->slug . '.css', array(), $WCFM->version );
				break;
			case 'wcfm-' . $this->slug . '-manage':
				wp_enqueue_style( 'collapsible_css', $WCFM->library->css_lib_url . 'wcfm-style-collapsible.css', array(), $WCFM->version );
				wp_enqueue_style( 'wcfm_' . $this->slug . '_manage_css', $this->plugin_url . 'css/' . $this->slug . '/wcfm-style-' . $this->slug . '-manage.css', array(), $WCFM->version );
				break;
		}
	}
	...
	public function load_scripts( $end_point ) {
		...
		switch ( $end_point ) {
			case 'wcfm-' . $this->slug:
				...
				wp_enqueue_script( 'wcfm_' . $this->slug . '_js', $this->plugin_url . 'js/' . $this->slug . '/wcfm-script-' . $this->slug . '.js', array( 'jquery', 'dataTables_js' ), $WCFM->version, true );

				$screen_manager = get_option( 'wcfm_screen_manager', array() );
				$data           = $screen_manager[ $this->slug ] ?? array();
			...
			case 'wcfm-' . $this->slug . '-manage':
				...
				wp_enqueue_script( 'wcfm_' . $this->slug . '_manage_js', $this->plugin_url . 'js/' . $this->slug . '/wcfm-script-' . $this->slug . '-manage.js', array( 'jquery' ), $WCFM->version, true );
