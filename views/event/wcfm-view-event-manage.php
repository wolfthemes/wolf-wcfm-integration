...
<div class="collapse wcfm-collapse" id="">
	...
	<div class="wcfm-collapse-content">
		...
		<div class="wcfm-container wcfm-top-element-container">
			...
			<?php
			if ( $event_id ) {
				_e( 'Edit Event', 'wcfm-cpt' );
			} else {
				_e( 'Add Event', 'wcfm-cpt' ); }
			?>
			</h2>
			<?php
			if ( $event_id ) {
				?>
				<span class="event-status event-status-<?php echo $wcfm_event_single->post_status; ?>">
				<?php
				if ( $wcfm_event_single->post_status == 'publish' ) {
					_e( 'Published', 'wcfm-cpt' );
				} else {
					_e( ucfirst( $wcfm_event_single->post_status ), 'wcfm-cpt' ); }
				?>
				</span>
				<?php
				if ( $wcfm_event_single->post_status == 'publish' ) {
		...
		<form id="wcfm_event_manage_form" class="wcfm">
				...
				<div id="wcfm_event_manage_form_general_expander" class="wcfm-content">
					<div class="wcfm_event_manager_general_fields">
						<?php
							$WCFM->wcfm_fields->wcfm_generate_form_field(
								apply_filters(
									'wcfm_event_manage_fields_general',
									array(
										'title' => array(
											'placeholder' => __( 'Event Title', 'wcfm-cpt' ),
											'type'        => 'text',
											'class'       => 'wcfm-text wcfm_event_title wcfm_full_ele',
											'value'       => $title,
										),
									),
									$event_id
						...
						<?php if ( $wcfm_is_category_checklist = apply_filters( 'wcfm_is_category_checklist', true ) ) { ?>
							...
							<div class="wcfm_event_manager_content_fields">
								...
								$WCFM->wcfm_fields->wcfm_generate_form_field(
									apply_filters(
										'wcfm_event_manage_fields_content',
										array(
					...
					<div class="wcfm_event_manager_gallery_fields">
						...
						if ( $wcfm_is_allow_featured = apply_filters( 'wcfm_is_allow_featured', true ) ) {
							$WCFM->wcfm_fields->wcfm_generate_form_field(
								apply_filters(
									'wcfm_event_manage_fields_gallery',
									array(
										'featured_img' => array(
											'type'        => 'upload',
											'class'       => 'wcfm-event-feature-upload',
			...
			<div class="wcfm-tabWrap">
				...
				<?php require 'wcfm-view-event-manage-tabs.php'; ?>
			...
			<div id="wcfm_event_simple_submit" class="wcfm_form_simple_submit_wrapper">
				...
				<?php if ( $event_id && ( $wcfm_event_single->post_status == 'publish' ) ) { ?>
					...
					?>
					" id="wcfm_event_simple_submit_button" class="wcfm_submit_button" />
				<?php } else { ?>
					<input type="submit" name="submit-data" value="
					<?php
