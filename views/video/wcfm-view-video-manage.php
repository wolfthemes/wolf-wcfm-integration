...
<div class="collapse wcfm-collapse" id="">
	...
	<div class="wcfm-collapse-content">
		...
		<div class="wcfm-container wcfm-top-element-container">
			...
			<?php
			if ( $video_id ) {
				_e( 'Edit Video', 'wcfm-cpt' );
			} else {
				_e( 'Add Video', 'wcfm-cpt' ); }
			?>
			</h2>
			<?php
			if ( $video_id ) {
				?>
				<span class="video-status video-status-<?php echo $wcfm_video_single->post_status; ?>">
				<?php
				if ( $wcfm_video_single->post_status == 'publish' ) {
					_e( 'Published', 'wcfm-cpt' );
				} else {
					_e( ucfirst( $wcfm_video_single->post_status ), 'wcfm-cpt' ); }
				?>
				</span>
				<?php
				if ( $wcfm_video_single->post_status == 'publish' ) {
		...
		<form id="wcfm_video_manage_form" class="wcfm">
				...
				<div id="wcfm_video_manage_form_general_expander" class="wcfm-content">
					<div class="wcfm_video_manager_general_fields">
						<?php
							$WCFM->wcfm_fields->wcfm_generate_form_field(
								apply_filters(
									'wcfm_video_manage_fields_general',
									array(
										'title' => array(
											'placeholder' => __( 'Video Title', 'wcfm-cpt' ),
											'type'        => 'text',
											'class'       => 'wcfm-text wcfm_video_title wcfm_full_ele',
											'value'       => $title,
										),
									),
									$video_id
						...
						<?php if ( $wcfm_is_category_checklist = apply_filters( 'wcfm_is_category_checklist', true ) ) { ?>
							...
							<div class="wcfm_video_manager_content_fields">
								...
								$WCFM->wcfm_fields->wcfm_generate_form_field(
									apply_filters(
										'wcfm_video_manage_fields_content',
										array(
											'description' => array(
												'label'       => __( 'Description', 'wcfm-cpt' ),
												'type'        => $wpeditor,
												'class'       => 'wcfm-textarea  wcfm_full_ele ' . $rich_editor,
												'label_class' => 'wcfm_title wcfm_full_ele ' . $rich_editor,
												'value'       => $description,
											),
											'excerpt'     => array(
												'label'       => __( 'Short Description', 'wcfm-cpt' ),
												'type'        => $wpeditor,
												'class'       => 'wcfm-textarea  wcfm_full_ele ' . $rich_editor,
												'label_class' => 'wcfm_title wcfm_full_ele ' . $rich_editor,
												'rows'        => 5,
					...
					<div class="wcfm_video_manager_gallery_fields">
						...
						if ( $wcfm_is_allow_featured = apply_filters( 'wcfm_is_allow_featured', true ) ) {
							$WCFM->wcfm_fields->wcfm_generate_form_field(
								apply_filters(
									'wcfm_video_manage_fields_gallery',
									array(
										'featured_img' => array(
											'type'        => 'upload',
											'class'       => 'wcfm-video-feature-upload',
										...
										foreach ( $video_taxonomies as $video_taxonomy ) {
											if ( ! in_array( $video_taxonomy->name, array( 'post_tag' ) ) ) {
												if ( $video_taxonomy->public && $video_taxonomy->show_ui && $video_taxonomy->meta_box_cb && ! $video_taxonomy->hierarchical ) {
													...
													$WCFM->wcfm_fields->wcfm_generate_form_field(
														array(
															$video_taxonomy->name => array(
																'label'       => $video_taxonomy->label,
																'name'        => 'video_custom_taxonomies_flat[' . $video_taxonomy->name . '][]',
																'type'        => 'textarea',
																'class'       => 'wcfm-textarea  wcfm_full_ele',
																'label_class' => 'wcfm_title wcfm_full_ele',
																'value'       => $taxonomy_values,
																'placeholder' => __( 'Separate video ' . $video_taxonomy->label . ' with commas', 'wcfm-cpt' ),
															),
														)
													);
												}
			...
			<div class="wcfm-tabWrap">
				...
				<?php require 'wcfm-view-video-manage-tabs.php'; ?>
			...
			<div id="wcfm_video_simple_submit" class="wcfm_form_simple_submit_wrapper">
				...
				<?php if ( $video_id && ( $wcfm_video_single->post_status == 'publish' ) ) { ?>
					...
					?>
					" id="wcfm_video_simple_submit_button" class="wcfm_submit_button" />
				<?php } else { ?>
					<input type="submit" name="submit-data" value="
					<?php
